...
var satellite = function () {
  ...
  function eci_to_geodetic(eci_coords, gmst) {
    ...
    var b = 6356.7523142;
    var R = Math.sqrt(
      eci_coords["x"] * eci_coords["x"] + eci_coords["y"] * eci_coords["y"]
    );
    var f = (a - b) / a;
    var e2 = 2 * f - f * f;
    ...
    while (k < kmax) {
      ...
      latitude = Math.atan2(
        eci_coords["z"] + a * C * e2 * Math.sin(latitude),
        R
      );
      k += 1;
    }
  ...
  function eci_to_ecf(eci_coords, gmst) {
    ...
    var Y =
      eci_coords["x"] * -Math.sin(gmst) + eci_coords["y"] * Math.cos(gmst);
    var Z = eci_coords["z"];
    return { x: X, y: Y, z: Z };
  }
  ...
  satellite.ecf_to_look_angles = function (
    observer_coords_ecf,
    satellite_coords_ecf
  ) {
    var topocentric_coords = topocentric(
      observer_coords_ecf,
      satellite_coords_ecf
    );
